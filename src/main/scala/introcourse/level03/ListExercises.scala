...
object ListExercises {
  ...
  /**
    ...
    * Hint: Use .map
    **/
  def addNumToEach(num: Int, nums: List[Int]): List[Int] = nums.map(x => x + num)
  ...
  /**
    ...
    */
  def filterEven(nums: List[Int]): List[Int] = nums.filter(x => x % 2 == 0)
  ...
  def min(nums: List[Int]): Int =
    nums match {
      ...
      case head :: tail => tail.foldLeft(head)((x, y) => if (x < y) x else y)
    }
  ...
  def youngestPerson(persons: List[Person]): Person = persons match {
    ...
    case head :: tail => tail.foldLeft(head)((x, y) => if (x.age <= y.age) x else y)
  }
  ...
  def reverseList[A](xs: List[A]): List[A] = {
    xs.foldLeft(List[A]())((list, x) => x :: list)
  }
  ...
  def sublists[A](xs: List[A]): List[List[A]] = {
    reverseList(xs.foldLeft(List[List[A]]()) { (list, x) =>
      list match {
        case (head :: tail1) :: tail2 if head == x => (x :: head :: tail1) :: tail2
        case _ => List(x) :: list
      }
    })
  }
