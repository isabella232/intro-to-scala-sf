...
object OptionExercises2 {
  ...
  def findJobDescriptionGivenJobId1(jobId: JobId): Option[String] = findJobById(jobId) match {
    case Some(j) => Some(j.description)
    case None => None
  }
  ...
  /**
    ...
    */
  def findJobDescriptionGivenJobIdOrElse2(jobId: JobId): String = findJobDescriptionGivenJobId1(jobId)
    .getOrElse(s"Job with id $jobId does not exist")
  ...
  /**
    ...
    */
  def findJobIdByHumanId(humanId: HumanId): Option[JobId] = findHumanById(humanId).map { human =>
    human.optJobId
  }.flatten
