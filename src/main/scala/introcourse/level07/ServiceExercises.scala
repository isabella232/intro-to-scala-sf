...
object Service {
   ...
   *   `userNames` method and return a response a HTTP OK status.
   ...
   *
   * Add to the pattern match, appropriately handling routes that are not found.
   *
   * Hints:
  ...
  /* Now write a new implementation for the users and point.
